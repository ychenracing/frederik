...
//			System.out.print("2");
	...
	public static void send(String FilePath,String MailAdd)
	{
		 ...
		 *************切切注意********
		 */
		mail themail = new mail("smtp.163.com");//这里以163邮箱为例子
		...
		themail.setSubject("The results from <http://www.iipl.fudan.edu.cn/FlyVar/>");//邮件主题
		themail.setBody(mailbody);//邮件正文
		themail.setTo(MailAdd);//收件人地址
		themail.setFrom("flyvar@163.com");//发件人地址
		String[] filepath_new=FilePath.split(" ");
		for(int i=0;i<filepath_new.length;i++)
			if(!"".equals(filepath_new[i]))
				themail.addFileAffix(filepath_new[i]);// 附件文件路径,例如：C:/222.jpg,*注；"/"的写法； 如果没有可以不写
		//themail.addFileAffix(FilePath.concat("_result"));
		themail.setNamePass("flyvar@163.com", "chenyong");//发件人地址和密码 **改为相应邮箱密码
		themail.sendout();
//		System.out.print("3");
...
//		System.out.print("3");
	...
	public static void send(String subject,String name,String contact,String content,String receiver)
	{
		...
		themail.setNeedAuth(true);
		themail.setSubject("feedback from frederik website\"Subject:"+subject+"\"");//邮件主题
		themail.setBody(mailbody);//邮件正文
		themail.setTo(receiver);//收件人地址
		themail.setFrom("flyvar@163.com");//发件人地址
		themail.setNamePass("flyvar@163.com", "chenyong");//发件人地址和密码 **改为相应邮箱密码
		themail.sendout();
