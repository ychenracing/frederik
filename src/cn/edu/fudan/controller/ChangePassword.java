...
public class ChangePassword extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (user == null) {
			String tips=URLEncoder.encode("error!Sorry to tell you that you should do it again!","utf-8");
			response.sendRedirect("en/Tips.jsp?tips="+tips);
		} else {
			if (!oldpassword.equals(user.getPassword())) {
				session.setAttribute("passworderror", "Old password error!");
				response.sendRedirect(url);
			}
			else{
				if(oldpassword.equals(newpassword)){
					String tips=URLEncoder.encode("Change password success!","utf-8");
					response.sendRedirect("en/Tips.jsp?tips="+tips);
					return;
				}
